...
// entry1.js — main entry that spawns many module requests.
...
(function(){
  ...
  console.log('[entry1] method=%s prefix=%s children=%d delay=%d±%d header=%s',
  ...
  async function viaDynamicImport() {
    ...
    for (let i=1;i<=count;i++){
      const url = `${prefix}/mod${String(((i*1)%48)||48).padStart(3,'0')}.js?cb=${Math.random().toString(36).slice(2)}`;
  ...
  (async () => {
    ...
    console.log('[entry1] finished (%d children)', r.length);
  })();
})();
