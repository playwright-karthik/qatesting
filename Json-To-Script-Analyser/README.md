...
Static browser tool for checking whether a frontend recorder JSON file was converted into the expected Playwright script, selectors, and optional Core runtime result.
...
- `recorder.json`
- `script.js`
- optional `diagnostic_results.json` or pasted Core diagnostic text

The page shows:

- summary counts
- plain-English interpretation and likely owner
- JSON verb to Playwright verb summary
- action-by-action mapping table
- selector fallback priority table
- Core/runtime step summary when diagnostic data is supplied
- findings
...
Internal partition/Core URLs are not fetched directly from GitHub Pages. Open the Core result separately and upload or paste the `diagnostic_results.json` content into the optional Core diagnostic field.
